    ...
    class AudioManager
    {
        ...
        public static Stream LoadFile(Stream file)
        {
            ...
            if (magic == SOUND_MAGIC_IDMSA)
            {
                ...
                index += 1;
                int playBegin = FileManager.FileManager.ReadInt32Swapped(file, index);
                index += 4;
                int playLength = FileManager.FileManager.ReadInt32Swapped(file, index);
                index += 4;
                ...
                int amplitudeNum = FileManager.FileManager.ReadInt32Swapped(file, index);
                index += 4;
                byte[] amplitudes = FileManager.FileManager.ReadByteArray(file, index, amplitudeNum);
                index += amplitudeNum;
                int totalBufferSize = FileManager.FileManager.ReadInt32Swapped(file, index);
                index += 4;
                data.Write(BitConverter.GetBytes(totalBufferSize), 0, 4);
                int bufferNum = FileManager.FileManager.ReadInt32Swapped(file, index);
                index += 4;
                GeneratedBuffer[] generatedBuffers = new GeneratedBuffer[bufferNum];
                for (int i = 0; i < bufferNum; i++)
                {
                    generatedBuffers[i] = new GeneratedBuffer();
                    generatedBuffers[i].numSamples = FileManager.FileManager.ReadInt32Swapped(file, index);
                    index += 4;
                    generatedBuffers[i].bufferSize = FileManager.FileManager.ReadInt32Swapped(file, index);
                    index += 4;
